...
[静态插画质感与遮挡诊断](skills/blender-character-workflow/references/static-illustration-finish.md) 包含图片纹理 UV 验收、保留腮红的肤色调整、眼面／虹膜／镜片分层检查，以及支撑面、内层与细丝的隔离诊断。[执行可靠性](skills/blender-character-workflow/references/blender-reliability.md) 补充原生曲线与着色模式输入检查、局部修改的状态保留和实时视口异常恢复。
...
- 图片全部使用 UV 不等于每根发丝都有独立绘制图集；零越界、零退化或有限穿插采样不代表无拉伸、笔刷可用或整个装配无穿模。报告须注明检查对象、方法及未覆盖部分。
